...
public class HistoricTaskDetailPanel extends DetailPanel {
	...
	private void initTasksEvent() {
		...
		TaskEventsPanel taskEventPanel = new TaskEventsPanel(true);
		taskEventPanel.setTaskId(historicTask.getId());
	...
	protected void initHeader() {
		...
		// Add due date
		PrettyTimeLabel dueDateLabel = new PrettyTimeLabel(
				i18nManager.getMessage(Messages.TASK_DUEDATE_SHORT),
				historicTask.getDueDate(),
		...
		case 1:
			priorityLabel.setValue(i18nManager
					.getMessage(Messages.TASK_PRIORITY_LOW));
			priorityLabel
					.addStyleName(ExplorerLayout.STYLE_TASK_HEADER_PRIORITY_LOW);
			break;
		case 2:
			priorityLabel.setValue(i18nManager
					.getMessage(Messages.TASK_PRIORITY_MEDIUM));
			priorityLabel
		...
		default:
			priorityLabel.setValue(i18nManager
					.getMessage(Messages.TASK_PRIORITY_HIGH));
			priorityLabel
	...
	protected void initParentTaskLink() {
		if (historicTask.getParentTaskId() != null) {
			final HistoricTaskInstance parentTask = historyService
	...
	protected void initOwner() {
		String ownerRole = historicTask.getOwner() != null ? Messages.TASK_OWNER
				: Messages.TASK_NO_OWNER;
		UserDetailsComponent owner = new UserDetailsComponent(
				historicTask.getOwner(), i18nManager.getMessage(ownerRole));
		peopleGrid.addComponent(owner);
	}

	protected void initAssignee() {
		String assigneeRole = historicTask.getAssignee() != null ? Messages.TASK_ASSIGNEE
				: Messages.TASK_NO_ASSIGNEE;
		UserDetailsComponent assignee = new UserDetailsComponent(
				historicTask.getAssignee(),
				i18nManager.getMessage(assigneeRole));
	...
	protected void initSubTasks() {
		...
		List<HistoricTaskInstance> subTasks = historyService
				.createHistoricTaskInstanceQuery()
				.taskParentTaskId(historicTask.getId()).list();
	...
	protected void populateSubTasks(List<HistoricTaskInstance> subTasks) {
		for (final HistoricTaskInstance subTask : subTasks) {
			...
			subTaskGrid.setComponentAlignment(subTaskLink,
	...
	protected void initNoSubTasksLabel() {
		Label noSubTasksLabel = new Label(
				i18nManager.getMessage(Messages.TASK_NO_SUBTASKS));
	...
	protected void initRelatedContent() {
		...
		List<Attachment> attachments = taskService
				.getTaskAttachments(historicTask.getId());
	...
	protected void initRelatedContentTitle() {
		Label title = new Label(ViewToolManager.getI18nManager().getMessage(
				Messages.TASK_RELATED_CONTENT));
		title.addStyleName(ExplorerLayout.STYLE_H3);
		title.setSizeFull();
	...
	protected void populateRelatedContent(Table table,
		...
		for (Attachment attachment : attachments) {
			...
			attachmentItem.getItemProperty("name").setValue(
					renderer.getOverviewComponent(attachment,
