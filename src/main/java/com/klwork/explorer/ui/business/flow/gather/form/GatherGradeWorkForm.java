...
public class GatherGradeWorkForm extends GatherPublishNeedForm {
	...
	private void initGrade() {
		...
		if(projectScoreParticipant == null){//创建一个临时的
			projectScoreParticipant = projectParticipantService.newProjectParticipantByParam(getRelateOutSourceingProject(), userId, EntityDictionary.PARTICIPANTS_TYPE_SCORER, getTask().getId());
			ProjectParticipant history = projectManagerService.queryUserParticipant(getRelateOutSourceingProject(), null,null);
			projectScoreParticipant.setScoreUserId(history.getUserId());//被评用户id
			projectScoreParticipant.setAssessedTaskId(history.getCurrentTaskId());//被评价任务
		}
