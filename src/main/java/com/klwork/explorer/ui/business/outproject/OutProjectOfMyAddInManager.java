...
public class OutProjectOfMyAddInManager extends AbstractVCustomComponent {
			...
			public void buttonClick(ClickEvent event) {
				try {
					...
					if(!teamService.checkExistName(LoginHandler.getLoggedInUser().getId(),team.getName())){
						...
						fireEvent(new SubmitEvent(OutProjectOfMyAddInManager.this,
	...
	public void initGroupTable(VerticalLayout tableLayout) {
		LazyLoadingQuery lazyLoadingQuery = new OutProjectPublishQuery(LoginHandler
		...
		if (lazyLoadingQuery.size() > 0) {
			...
			Label noMembersTable = new Label(
					i18nManager.getMessage(Messages.OUTPROJECT_NO_PUBLISH_PROMPT));
