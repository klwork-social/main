...
public class ParticipationGroupListRight extends DetailPanel {
	...
	protected ParticipationOrganizationMainPage mainPage;

	public ParticipationGroupListRight(
			ParticipationOrganizationMainPage participationOrganizationMainPage,
		...
		mainPage = participationOrganizationMainPage;
	}
	...
	public class MemberActionsColumnGenerator implements ColumnGenerator {
		...
		public Object generateCell(final Table source, final Object itemId,
				Object columnId) {
			final UserEntity userEntity = BinderHandler.getTableBean(source,
					itemId);
			if (userEntity.getId().equals(
					LoginHandler.getLoggedInUser().getId())) {// 是当前用户
				
				Button deleteButton = new Button("取消加入");
				...
				deleteButton.addClickListener(new ClickListener() {
					public void buttonClick(ClickEvent event) {
						ConfirmationDialogPopupWindow confirmationPopup = new ConfirmationDialogPopupWindow(
								i18nManager
										.getMessage(
												Messages.USER_CONFIRM_DELETE_GROUP,
												userEntity.getId(),
												team.getName()));
						confirmationPopup
								.addListener(new ConfirmationEventListener() {
									...
									protected void confirmed(
											ConfirmationEvent event) {
										teamMembershipService.deleteTeamMembership(
												userEntity.getId(),
												team.getId());
						...
						ViewToolManager
								.showPopupWindow(confirmationPopup);
					}
				});
				return deleteButton;
			} else {
