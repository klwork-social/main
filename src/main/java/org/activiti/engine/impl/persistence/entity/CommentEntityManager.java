...
import org.activiti.engine.impl.EventQueryImpl;
...
public class CommentEntityManager extends AbstractManager {
	...
	public List<Event> findAllEvents() {
		checkHistoryEnabled();
		return getDbSqlSession().selectList("selectAllEvents");
	}

	@SuppressWarnings("unchecked")
	public List<Task> findEventByQueryCriteria(TaskQueryImpl taskQuery) {
		final String query = "selectEventByQueryCriteria";
	...
	public long findEventCountByQueryCriteria(EventQueryImpl eventQuery) {
		 return (Long) getDbSqlSession().selectOne("selectEventCountByQueryCriteria", eventQuery);
	}
	
	@SuppressWarnings("unchecked")
	public List<Event> findEventByQueryCriteria(EventQueryImpl eventQuery,
			Page page) {
		if(page == null){
			page = new Page(0,10);
		}
		eventQuery.setFirstResult(page.getFirstResult());
		eventQuery.setMaxResults(page.getMaxResults());
		final String query = "selectEventByQueryCriteria";
	    return getDbSqlSession().selectList(query, eventQuery);
	}
}
